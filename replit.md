...
- November 4, 2025. Implemented dynamic lesson linking system for dive sites. Added linkedLessonId field to dive_sites schema allowing any dive site to link to relevant educational content. Updated DiveSiteDetails component to display linked lessons dynamically with "Start Learning →" button that navigates to the lesson with auto-open functionality. Connected Camilla Wreck and Long Jetty dive sites to their maritime history lessons.
- November 4, 2025. Recategorized whale lessons to Marine Mammals category: Moved "Whale Science 101: Tracking Giants", "Southern Right Whales: Migration Mysteries", and "Southern Right Whales: Tracking Climate Change" from their previous categories (marine-research, species-identification, conservation) to marine-mammals for better lesson organization.
- November 4, 2025. Added "Bunbury's Bottlenose: The City Where Dolphins Meet the Shore" lesson to Marine Mammals category. Covers Indo-Pacific bottlenose dolphins in Koombana Bay, Bunbury, including dolphin-human coexistence, conservation challenges, and the Dolphin Discovery Centre's role. Follows enhanced lesson format with 3 quizzes, final quiz, action items, and sources from WA government and heritage organizations. Total lesson count now at 23.
- November 6, 2025. Implemented comprehensive lesson progress tracking system with PostgreSQL persistence. Added lesson_progress and category_badges database tables. Created CircularProgress component with ocean-themed planet indicator showing completion percentage and lesson count. Built BadgeShowcase component displaying earned category badges. Progress tracker is now category-specific: shows overall progress when "All Categories" selected, and category-filtered progress when a specific category is selected. Integrated "Mark Complete" button in EnhancedLessonViewer that saves progress to database and shows toast notifications. Expanded badge system to include all 17 categories with unique icons: Ocean Literacy (🌊), Reef Ecology (📘), Species ID (🧬), Conservation (🌱), Marine Research (🧪), Maritime History (📜), Marine Mammals (🐋), Human-Ocean Interaction (🤝), Basic Oceanic Physics (⚛️), Diving Ethics (🤿), Biology Knowledge (🧫), Deep Sea (🌌), Estuarine Environments (🌾), Cephalopods (🐙), Marine Ecology (🪸), Marine Geology (🪨), and Ocean Energy (⚡). Badge unlocking is automatic when all lessons in a category are completed. Progress tracking supports cross-device synchronization via database storage.
