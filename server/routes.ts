...
const createPostRequestSchema = insertPostSchema.extend({
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/dive-sites", async (req: Request, res: Response) => {
    try {
      ...
      if (req.query.minDepth)
        filters.minDepth = parseInt(req.query.minDepth as string);
      if (req.query.maxDepth)
        filters.maxDepth = parseInt(req.query.maxDepth as string);
  ...
  app.post("/api/dive-sites", async (req: Request, res: Response) => {
    try {
      ...
      if (error instanceof Error && error.name === "ZodError") {
        return res
          .status(400)
          .json({ error: "Invalid dive site data", details: error.message });
      }
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to fetch species for dive site" });
      }
    },
  );
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to fetch reviews for dive site" });
      }
    },
  );
  ...
  app.get(
    "/api/dive-sites/:id/dive-centers",
    async (req: Request, res: Response) => {
      try {
        const diveSiteId = parseInt(req.params.id);
  ...
  app.get("/api/species/:id/images", async (req: Request, res: Response) => {
    try {
      const speciesId = parseInt(req.params.id);
  ...
  app.post("/api/species/:id/images", async (req: Request, res: Response) => {
    try {
      if (!req.session?.userId) {
      ...
      if (isNaN(speciesId) || isNaN(imageId)) {
        return res.status(400).json({ error: "Invalid species or image id" });
      }
      ...
      console.error("Error linking species image:", error);
      res.status(500).json({ error: "Failed to link image to species" });
    }
  });
  ...
  app.post("/api/dive-logs", async (req: Request, res: Response) => {
    try {
      ...
      if (
        req.body.species &&
        Array.isArray(req.body.species) &&
  ...
  app.put("/api/dive-logs/:id", async (req: Request, res: Response) => {
    try {
      ...
      const validationResult = insertDiveLogSchema
        .partial()
      ...
      const updatedDiveLog = await storage.updateDiveLog(
        diveLogId,
  ...
  app.post("/api/spotted-species", async (req, res) => {
    try {
      const spottedData = validateRequest(
        insertUserSpottedSpeciesSchema,
        req.body,
      );
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!conditions) {
          return res
            .status(404)
            .json({ error: "No water conditions found for this dive site" });
        }
  ...
  app.get(
    "/api/dive-sites/:id/live-conditions",
    async (req: Request, res: Response) => {
      try {
        const diveSiteId = parseInt(req.params.id);
        if (isNaN(diveSiteId)) {
        ...
        const [oceanData, weatherData] = await Promise.all([
          OceanDataService.getLiveOceanData(
            diveSite.latitude,
            diveSite.longitude,
          ),
          OceanDataService.getWeatherData(
            diveSite.latitude,
            diveSite.longitude,
          ),
        ]);
        ...
        const liveConditions = {
          ...
          currentStrength: oceanData
            ? `${oceanData.currentSpeed.toFixed(1)} cm/s`
            : null,
  ...
  app.get(
    "/api/dive-sites/:id/conditions/history",
    async (req: Request, res: Response) => {
      try {
        const diveSiteId = parseInt(req.params.id);
        if (isNaN(diveSiteId)) {
        ...
        const conditions = await storage.getWaterConditionsHistory(
          diveSiteId,
          days,
        );
        res.json(conditions);
        ...
        res
          .status(500)
          .json({ error: "Failed to fetch water conditions history" });
      }
    },
  );
  ...
  app.post("/api/users/certifications", async (req: Request, res: Response) => {
    try {
      ...
      if (!req.body.certificationId) {
        return res
          .status(400)
          .json({ success: false, message: "Certification ID is required" });
      }
      ...
      const certification = await storage.getCertification(
      ...
      const userCertification = await storage.addUserCertification(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (isNaN(certificationId)) {
          return res
            .status(400)
            .json({ success: false, message: "Invalid certification ID" });
        }

        const success = await storage.removeUserCertification(
          certificationId,
          req.session.userId,
        );
        if (!success) {
        ...
        res
          .status(500)
          .json({ success: false, message: "Failed to remove certification" });
      }
    },
  );
  ...
  app.post("/api/progress/:lessonId", async (req: Request, res: Response) => {
    try {
      ...
      const lessonProgress = await storage.markLessonComplete(
        req.session.userId,
        lessonId,
      );

      // Check if this completion unlocks a category badge
      const badge = await storage.checkAndUnlockBadge(
        req.session.userId,
        lessonId,
      );
      ...
      if (error.code === "23505") {
        ...
        return res
          .status(400)
          .json({ success: false, message: "Lesson already completed" });
      }
  ...
  app.get("/api/badges", async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, message: "Failed to fetch badges" });
    }
  });
  ...
  app.post("/api/posts", async (req: Request, res: Response) => {
    try {
      ...
      const validationResult = createPostRequestSchema.safeParse({
        ...req.body,
        userId: req.session.userId,
      });
      ...
      let { imageIds = [], speciesIds = [], ...postPayload } = validationResult.data as any;
      imageIds = Array.isArray(imageIds) ? imageIds.filter((id: any) => typeof id === "number") : [];
      speciesIds = Array.isArray(speciesIds) ? speciesIds.filter((id: any) => typeof id === "number") : [];

      // If an inline photoUrl was provided but no imageIds, mirror it into the images table
      if (postPayload.photoUrl && imageIds.length === 0) {
        const image = await storage.createImage({
          url: postPayload.photoUrl,
          alt: "Post photo",
          userId: req.session.userId,
          source: "inline-upload",
        });
        imageIds.push(image.id);
        postPayload.photoUrl = image.url;
      }

      // If the caller uploaded images first, set the post preview URL from the first image
      if (!postPayload.photoUrl && imageIds.length > 0) {
        const primaryImage = await storage.getImage(imageIds[0]);
        if (primaryImage) {
          postPayload.photoUrl = primaryImage.url;
        }
      }

      postPayload.primaryImageId = postPayload.primaryImageId ?? imageIds[0] ?? null;

      const post = await storage.createPost(postPayload);

      if (imageIds.length > 0) {
        await storage.addPostImages(post.id, imageIds);
      }

      if (speciesIds.length > 0) {
        await storage.addPostSpecies(post.id, speciesIds);
      }

      const enrichedPost =
        (await storage.getAllPosts()).find((p: any) => p.id === post.id) || post;

      res.status(201).json({ success: true, post: enrichedPost });
    } catch (error) {
